...
It downloads the lsit of oncogenes from: http://ongene.bioinfo-minzhao.org/

Original article: http://dx.doi.org/10.1016/j.jgg.2016.12.004
...
if __name__ == '__main__':
    ...
    url = 'http://ongene.bioinfo-minzhao.org/ongene_human.txt'
    tmp_file = os.path.join(options.output_directory,'temp_oncogenes.tsv')
    ...
    file(os.path.join(options.output_directory,'oncogenes_more.txt'),'w').write('')

    # save version of
    today = datetime.date.today()
    txt = ['Oncogenes database version: %s\n' % (today.strftime("%Y-%m-%d"),)]
    ...
    if options.organism.lower() == 'homo_sapiens':
        ...
        if sem:
            ...
            d = [line.upper().rstrip("\r\n").split("\t")[1] for line in file(tmp_file,"r")]
        ...
        file(os.path.join(options.output_directory,'oncogenes_more.txt'),'w').writelines(data)
