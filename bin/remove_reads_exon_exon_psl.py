...
def fast_join(a_list):
    return "%s-%s" % (a_list[0],a_list[1])
...
if __name__ == '__main__':
    ...
    parser.add_option("--input_psl",
                      action = "store",
                      type = "string",
                      dest = "input_psl_filename",
                      help = """The input PSL file (output of BLAT) containing
                             the reads mapping gene-gene sequence.""")
    ...
    print "Reading PSL...",options.input_psl_filename
    exjuncs = [line.rstrip('\r\n').split('\t') for line in file(options.input_psl_filename,'r').readlines() if line.rstrip('\r\n')]
    ex = dict()
    for line in exjuncs:
        r = line[9] # read in gene-gene sequence
        m = r[:-1]+'2' if r.endswith('/1') else r[:-1]+'1' # in transcriptome
        g2 = line[13].split('|')[:2] # ENSG00000108443|ENSG00000159210|57479
        g = fast_join([g2[0],g2[1]])
    ...
    for r in alk:
        ...
        m = ex[r]['mate']
        if ex.has_key(m):
            # erase both reads because they form a pair and both reads are unmapped
            del ex[r]
            del ex[m]
        ...
        for (k1,v1) in x.items():
            for (k2,v2) in v1.items():
                ...
                if ((not v2['erase']) and (k1 not in genes) and (v2['pair'] not in genes)):
                        ...
                        i = i + 1
                        #print "The read %s from exon-exon-junc (gene-gene: %s) has the mate %s which maps on transcriptome on genes: %s" % (ex[a_read]['mate'],k2,a_read,','.join(genes))
        ex[a_read]['gene'] = x
    ...
    fi = open(options.input_psl_filename,'r')
    fo = open(options.output_psl_filename,'w')
    while True:
        ...
        lines = [line for line in lines if  (line[9] in on_transcriptome and # read; its mate is found to map on transcriptome
                                             not (erase.has_key(line[9]) and # read
                                             fast_join(line[13].split('|',2)[:2]) in erase[line[9]])) # junction
                                            ]
        #
        lines = ['\t'.join(line)+'\n' for line in lines]
        fo.writelines(lines)
