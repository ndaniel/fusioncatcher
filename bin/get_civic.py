...
# -*- coding: utf-8 -*-
"""
It downloads the lastest known fusion genes from CIViC repository 
<https://github.com/griffithlab/civic-server/>.
...
if __name__ == '__main__':
    ...
    usage = "%prog [options]"
    description = """It downloads the lastest known fusion genes from CIViC repository <https://github.com/griffithlab/civic-server/>."""
    ...
    tmp_file = 'temp_civic.txt'

    url = '/griffithlab/civic-server/raw/master/public/downloads/ClinicalEvidenceSummaries.tsv'

    if options.organism.lower() == 'homo_sapiens':
        today = datetime.date.today()
        file(os.path.join(options.output_directory,'version.txt'),'a').writelines(['CIViC: %s\n' % (today.strftime("%Y-%m-%d"),)])
        ...
        if sem:
            ...
            for line in file(tmp_file,'r').readlines():
                li = line.rstrip('\r\n').split('\t')
                fus = li[2]
                if fus in ("LOSS-OF-FUNCTION","TERMINAL FRAME SHIFT","N","N-TERMINAL FRAME SHIFT"):
                    continue
                if fus.find("-") == -1:
                    continue
                fus = fus.split("-")
            ...
            data = []
        file(os.path.join(options.output_directory,'civic.txt'),'w').writelines(data)
        if os.path.isfile(tmp_file):
            os.remove(tmp_file)
    else:
        # write an empty file for other organisms than human
        file(os.path.join(options.output_directory,'civic.txt'),'w').write('')
#
