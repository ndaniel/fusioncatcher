...
P. Bailey et al., Genomic analyses identify molecular subtypes of pancreatic cancer, Nature, Feb. 2016, http://dx.doi.org/110.1038/nature16965
...
if __name__ == '__main__':
    ...
    usage = "%prog [options]"
    description = """It downloads the human fusion genes found in pancreatic cancer patients from article: P. Bailey et al., Genomic analyses identify molecular subtypes of pancreatic cancer, Nature, Feb. 2016, http://dx.doi.org/110.1038/nature16965"""
    ...
    url = 'http://www.nature.com/nature/journal/vaop/ncurrent/extref/nature16965-s1.xlsx'
    tmp_file = os.path.join(options.output_directory,'temp_pancreases.xlsx')
...
#    headers = { 'User-Agent' : 'Mozilla/5.0' }
    ...
    if options.organism.lower() == 'homo_sapiens':
        ...
        if sem:
            ...
            sheet = wb.get_sheet_by_name(name = 'Supp Table 13 In frame fusions') #
            ...
            for i,row in enumerate(sheet.rows): # sheet.iter_rows()
                ...
                try:
                    ...
                    g2 = row[2].value.encode('ascii','ignore').upper().strip()
                except:
                    f = True
                if f:
                    continue
            ...
            if not options.skip_filter_overlap:
                ...
                for ov in overlappings:
                    ...
                    if os.path.isfile(p):
                        ...
                        d4 = ['\t'.join(line)+'\n' for line in d4]
                        file(os.path.join(options.output_directory,"pancreases___%s"%(ov,)),'w').writelines(d4)
                    if ov in ensembls:
                        ens.extend(d2)
                    d1.extend(d2)
                d = set()
                ...
                file(os.path.join(options.output_directory,'pancraeses___ensembl.txt'),'w').writelines(sorted(ensembl))
                skipped = [line for line in data if line in d]
                data = [line for line in data if line not in d]
                file(os.path.join(options.output_directory,'pancreases___all.txt'),'w').writelines(sorted(skipped))

                print "%d known fusion genes left after removing the overlappings" % (len(data),)

        file(os.path.join(options.output_directory,'pancreases.txt'),'w').writelines(data)
    ...
    txt = ['Pancreatic Tumor Patients (Bailey et al. Nature 2016) database version: %s\n' % (today.strftime("%Y-%m-%d"),)]
