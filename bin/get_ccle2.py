...
if __name__ == '__main__':
    ...
    try:
        ...
        file(os.path.join(options.output_directory,'ccle2.txt'),'w').write('')
        sys.exit(0)
            ...
            if not options.skip_filter_overlap:
                ...
                for ov in overlappings:
                    ...
                    if os.path.isfile(p):
                        ...
                        d4 = ['\t'.join(line)+'\n' for line in d4]
                        file(os.path.join(options.output_directory,"ccle2___%s"%(ov,)),'w').writelines(d4)
                    if ov in ensembls:
                        ens.extend(d2)
                    d1.extend(d2)
                d = set()
                ...
                file(os.path.join(options.output_directory,'ccle2___ensembl.txt'),'w').writelines(sorted(ensembl))
                skipped = [line for line in data if line in d]
                data = [line for line in data if line not in d]
                file(os.path.join(options.output_directory,'ccle2___all.txt'),'w').writelines(sorted(skipped))

                print "%d known fusion genes left after removing the overlappings" % (len(data),)

        file(os.path.join(options.output_directory,'ccle2.txt'),'w').writelines(data)
