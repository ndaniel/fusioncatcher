...
- STAR version 2.4.0f <http://code.google.com/p/rna-star/>. Executables are
...
def is_known_extension(something):
    kx = ['.txt.gz','.fastq.gz','.fq.gz','.txt.bz2','.fastq.bz2',
          '.fq.bz2','.txt.zip','.fastq.zip','.fq.zip','.txt', '.fastq',
          '.fq','.sra','.bam','.sam']
    sign = False
    for ekx in kx:
...
if __name__ == "__main__":
    ...
    parser.add_option("--input","-i",
                      ...
                      help = "The input file(s) or directory. The files should be "+
                             ...
                             ".fastq, .fastq.zip, .fastq.gz, .fastq.bz2, "+
                             ".fq, .fq.zip, .fq.gz, .fq.bz2, "+
    ...
    parser.add_option("--config",
                      ...
                      default = os.path.join(pipeline_path,"configuration.cfg"),
    ...
    group.add_option("--5keep","-l",
                      ...
                      dest = "trim_3end_keep",
                      default = 60,
                      help = "All reads longer than '%default' will be trimmed from "+
    ...
    group.add_option("--trim-psl-5keep","-x",
                      ...
                      default = 80,
                      help = "All reads given as input to BLAT/STAR/BOWTIE2 aligners and which "+
    ...
    group.add_option("--pairs-fusion","-s",
                      ...
                      help = "The minimum number of paired-end reads which "+
                             ...
                             "the aligners: BOWTIE,BLAT,STAR,BOWTIE2 (in this order). " +
                             "Default is '%default'.")
    ...
    group.add_option("--anchor-fusion","-a",
                      action = "store",
                      dest = "length_anchor",
                      default = "17,17,17,23", # default 14; 17; 18?
                      help = "The minimum length which a read should overlap over (or anchor/overhang for) "+
                             ...
                             "by commas for each of the aligners: BOWTIE,BLAT,STAR,BOWTIE2 (in this order). " +
                             "Default is '%default'.")
    ...
    group.add_option("--homolog-fusion","-H",
                      ...
                      type = "float",
                      dest = "homolog",
                      default = float(1)/float(8*(10**4)),# float(1)/float(5*(10**4)),
                      help = "The minimum number of reads (as percentage [0..1]) "+
                             ...
                             "This information can be used for filtering out candidate "+
    ...
    group.add_option("--skip-filter-ambigous",
                      action = "store_true",
                      dest = "skip_ambigous_filtering",
                      default = False,
                      help = "If specified then it skips filtering out the reads which "+
                             "maps ambigously (i.e. same read map simultaneously on two "+
    ...
    group.add_option("--skip-readthroughs",
                      action = "store_true",
                      dest = "skip_readthroughs",
                      default = False,
                      help = "If it is set then the pipeline will not seach for "+
                             "candidate fusion genes which are readthroughs! "+
                             "By default the candidate fusion genes which are "+
                             "readthroughs are analysed also but in many cases "+
                             "they are just annotation errors in the Ensembl database "+
                             "and not real fusion genes. "+
                             "Default is '%default'.")
    ...
    group = optparse.OptionGroup(parser, "Usages of aligners")
    ...
    group.add_option("--aligners",
                      ...
                      help = "The aligners to be used on Bowtie aligner. "+
                             ...
                             "For example, if one wants to used all three aligners "+
    ...
    group.add_option("--limit-star",
                      ...
                      help = "The maximum limit of the genome's size which STAR aligner "+
                             "is able to handle.  If the genome is larger than this limir "+
                             "then it will be split automatically in smaller pieces such that "+
                             "the aligner can handle them without an error. "+
                             "Default is '%default'.")
    ...
    group = optparse.OptionGroup(parser, "Search sensitivity of fusions")
    ...
    group = optparse.OptionGroup(parser, "Custom labels for fusions and filtering of fusions")
...
#                      default = 2*(10**9),
#                      help = "The size of the buffer used for extracting reads "+
#                             "from a Fastq file when given a list of reads ids. "+
#                             "This operation requires a of free RAM memory "+
#                             "and on so computers with low memory this might "+
#                             "be required to be lowered. " +
#                             "Default is '%default'.")
    ...
    if (
        ...
        print ""
        print ""
        print "ERROR: input/output directory is not specified!"
        sys.exit(1)
    ...
    #
    paths = configuration.manage(options.configuration_filename,skip_python=['openpyxl','xlrd'])
    if not options.data_directory:
        p = paths.get("DATA",None)
        if p and (os.path.isdir(p) or os.path.islink(p)):
            options.data_directory = p
        else:
            parser.error("ERROR: Argument '--data' needs to be specified as command line or in 'configuarion.cfg' file!")
            sys.exit(1)
    ...
    if options.skip_readthroughs and options.biotypes.find('readthrough') == -1:
        options.biotypes = options.biotypes + ',readthrough'
    ...
    if (not os.path.isdir(out_dir)) and (not os.path.islink(out_dir)):
    ...
    if options.sensitive:
        options.spanning_pairs = "2,2,2,2"
        options.spanning_reads = "2,2,2,2"
        options.length_anchor = "14,17,17,22"
    ...
    if options.mildly_sensitive:
        options.spanning_pairs = "2,2,2,2"
        options.spanning_reads = "2,2,2,2"
        options.length_anchor = "13,15,15,22"
    ...
    if options.highly_sensitive:
        options.spanning_pairs = "2,2,2,2"
        options.spanning_reads = "1,1,1,1"
        options.length_anchor = "13,14,14,20"
        options.length_anchor2 = 22
        options.mismatches = 2
        options.mismatches_psl = 4
    ...
    if options.paranoid_sensitive:
        options.spanning_pairs = "2,2,2,2"
        options.spanning_reads = "1,1,1,1"
        options.length_anchor = "11,11,11,20"
        options.length_anchor2 = 22
        options.mismatches = 2
        options.mismatches_psl = 4
...
#        options.all_reads_junction = True
    ...
    if len(spanning_pairs) != 4:
        print >>sys.stderr, "ERROR: the command option SPANNING_PAIRS has been given incorrectly! Expecting 3 commas!"
        sys.exit(1)
    ...
    if len(spanning_reads) != 4:
        print >>sys.stderr, "ERROR: the command option SPANNING_READS has been given incorrectly! Expecting 3 commas!"
        sys.exit(1)
    ...
    if len(length_anchor) != 4:
        print >>sys.stderr, "ERROR: the command option LENGTH_ANCHOR has been given incorrectly! Expecting 3 commas!"
        sys.exit(1)
    ...
    if spanning_reads_minimum < 1:
        parser.error("ERROR: The minimum value of the SPANNING_READS is 1 but the value %d was given!" % (options.spanning_reads,))
        sys.exit(1)

    if length_anchor_minimum < 10:
        parser.error("ERROR: The minimum value of the LENGTH_ANCHOR is 10 but the value %d was given!" % (options.length_anchor,))
        sys.exit(1)

    if length_anchor2 <= length_anchor_minimum:
        parser.error("ERROR: --anchor-fusion2 (%d) should be larger than anchor-fusion (%d)!" % (options.length_anchor2,options.length_anchor2))
        sys.exit(1)

    if spanning_pairs_bowtie != spanning_pairs_minimum or spanning_pairs_bowtie < 2:
        parser.error("ERROR: The minimum value of the SPANNING_PAIRS should have values larger than 2 but the value %d was given!" % (options.spanning_pairs,))
        sys.exit(1)

    if length_anchor_bowtie2 < 20:
        parser.error("ERROR: The minimum value of the SPANNING_READS_BOWTIE2 should be larger (or equal) than 20 but the value %d was given!" % (length_anchor_bowtie2,))
        sys.exit(1)
    ...
    if job.run():
        if not os.path.isdir(tmp_dir) and not os.path.islink(tmp_dir):
            os.makedirs(tmp_dir)
...
#    job.run()
    ...
    info(job,
         fromfile = options.configuration_filename,
         tofile = info_file,
         top = ["===================",
                "CONFIGURATION.CFG:",
                "==================="],
        ...
        if os.path.isdir(element):
            ...
            if element.find('ssh://') != -1:
                ...
                if element.endswith('/') or element.endswith(os.sep):
                    ...
                    job.add('-A','txt,txt.gz,fq.gz,fastq.gz,txt.zip,fq.zip,fastq.zip,txt.bz2,fq.gz,fastq.bz2,sra',kind='parameter')
...
#
            ...
            job.add('-jar','SamToFastq.jar',kind='parameter')
    ...
    for i,input_file in enumerate(list_input_files):
        ...
        if input_file.lower().endswith('.gz') and (not input_file.lower().endswith('.tar.gz')):
            ...
            job.add('>',output_file,kind='output')
            job.run()
        elif (input_file.lower().endswith('.txt') or
              input_file.lower().endswith('.fq') or
...
#        job.run()
    ...
    if job.run():
        ...
        # short reads shorter than this will be skipped from analysis, 34?
        minimum_length_short_read = str(len_reads)
        file(outdir('log_minimum_length_short_read.txt'),'w').write(str(minimum_length_short_read))
        if (2 * length_anchor_minimum) > len_reads - 10:
...
#    job.run()
    ...
    #input_file = outdir('reads.fq')
    #output_file = outdir('reads_b2n.fq')
    #if not options.skip_b_filtering:
    # fix Illumina "B"
    job.add('fastq_b2n.py',kind='program')
    #job.add('--input',input_file,kind='input',temp_path=temp_flag)
    job.add('--input',outdir('reads.fq'),kind='input',temp_path=temp_flag)
    ...
    if pigz:
        ...
        job.add('gzip',kind='program')
    job.add('-c',outdir('original.fq'),kind='input',temp_path=temp_flag)
    job.add('>',outdir('original.fq.gz'),kind='output')
    job.run()
...
#        job.run()
    ...
    info(job,
        ...
        tofile = info_file,
        top = ["Count of all short reads after removing reads due to missing their mate read:",
               "-----------------------------------------------------------------------------"],
        bottom = "\n\n\n")
    ...
    if not options.skip_automatic_scaling:
        if max_len_reads < 60:
            ...
            job.add('printf',kind='program')
            job.add(('"\nInput reads are too short (maxim found length is %d) in order to use BOWTIE2 and therefore BOWTIE2 method is disabled automatically!\n"') % (max_len_reads,),kind='parameter')
            job.add('>>',info_file,kind='output')
            job.run()
        ...
        if (no_reads and
            ...
            if no_reads < 35000000 and no_reads >= 15000000 and max_len_reads < 60:
                ...
                spanning_pairs_minimum = min([spanning_pairs_bowtie,spanning_pairs_blat,spanning_pairs_star,spanning_pairs_bowtie2])
                spanning_reads_bowtie = 2
                spanning_reads_minimum = min([spanning_reads_bowtie,spanning_reads_blat,spanning_reads_star,spanning_reads_bowtie2])
                length_anchor_bowtie = 14
                job.add('printf',kind='program')
                job.add('"\nAdjusted automatically spanning_pairs, spanning_reads, and length_anchor for count of reads [15000000,3500000) and reads shorter than 60bp.\n\n"',kind='parameter')
                job.add('>>',info_file,kind='output')
                job.run()
            if no_reads < 15000000 and no_reads >= 1000000 and max_len_reads < 60:
                ...
                job.add('printf',kind='program')
                job.add('"\nAdjusted automatically spanning_pairs, spanning_reads, and length_anchor for count of reads [1000000,15000000) and reads shorter than 60bp.\n\n"',kind='parameter')
                job.add('>>',info_file,kind='output')
                job.run()
            if no_reads < 20000000 and no_reads >= 1000000 and max_len_reads > 74:
                ...
                spanning_reads_minimum = min([spanning_reads_bowtie,spanning_reads_blat,spanning_reads_star,spanning_reads_bowtie2])
                job.add('printf',kind='program')
                job.add('"\nAdjusted automatically spanning_pairs, spanning_reads, and length_anchor for count of reads [1000000,20000000) and reads longer than 74 bp.\n\n"',kind='parameter')
                job.add('>>',info_file,kind='output')
                job.run()
...
#    job.clean(outdir('log_reads2_removed.txt'),temp_path=temp_flag)
    ...
    job.add('-v','3',kind='parameter') #options.mismatches
    job.add('-p',options.processes,kind='parameter',checksum='no')
    #job.add('-m','1',kind='parameter')
    job.add('-k','1',kind='parameter')
    job.add('--solexa1.3-quals',kind='parameter')
    job.add('--tryhard',kind='parameter')
    job.add('--chunkmbs','128',kind='parameter')
    #job.add('--best',kind='parameter')
    #job.add('--strata',kind='parameter')
    job.add('--trim3','4',kind='parameter') # trim on the fly 5bp from 3' end
    job.add('--trim5','4',kind='parameter') # trim the 5
...
#    job.run()
    ...
    job.add('--best',kind='parameter')
    job.add('--strata',kind='parameter')
    job.add('--tryhard',kind='parameter') # ??? really necessary?
    job.add('--chunkmbs','128',kind='parameter')
...
#    job.run()
    ...
    #job.add('--input',outdir('reads-filtered.fq'),kind='input',temp_path=temp_flag)
    #job.add('--list',outdir('list-names-reads-filtered_genome.txt'),kind='input',temp_path=temp_flag)
    ...
    job.add('',outdir('reads-filtered.fq'),kind='input',temp_path=temp_flag)
    job.add('',outdir('list-names-reads-filtered_genome.txt'),kind='input',temp_path=temp_flag)
...
#    job.add('extract_short_reads.py',kind='program')
#    job.add('--input',outdir('reads_filtered_not-mapped-genome.fq'),kind='input',temp_path=temp_flag)
#    job.add('--list',outdir('list-names-reads-filtered_not-mapped-genome_mapped-transcriptome.txt'),kind='input',temp_path=temp_flag)
...
#                             "of FusionCatcher and running it again might help!"))
    ...
    job.add('seqtk',kind='program')
    job.add('subseq',kind='parameter')
    job.add('',outdir('reads_filtered_not-mapped-genome.fq'),kind='input',temp_path=temp_flag)
    job.add('',outdir('list-names-reads-filtered_not-mapped-genome_mapped-transcriptome.txt'),kind='input',temp_path=temp_flag)
    ...
    # filter the mapped reads on transcriptome with mapped on genome using mismatches
    job.add('remove_reads_map_genome_transcriptome.py',kind='program')
...
#    extract the short reads which mapped on the transcriptome and do not map on genome
#    job.add('extract_short_reads.py',kind='program')
#    job.add('--input',outdir('reads_filtered_unique-mapped-genome.fq'),kind='input',temp_path=temp_flag)
#    job.add('--list',outdir('list-names-reads-filtered_unique-mapped-genome_mapped-transcriptome.txt'),kind='input',temp_path=temp_flag)
...
#                             "of FusionCatcher and running it again might help!"))
    ...
    job.add('seqtk',kind='program')
    job.add('subseq',kind='parameter')
    job.add('',outdir('reads_filtered_unique-mapped-genome.fq'),kind='input',temp_path=temp_flag)
    job.add('',outdir('list-names-reads-filtered_unique-mapped-genome_mapped-transcriptome.txt'),kind='input',temp_path=temp_flag)
...
#    job.add('>',outdir('reads_filtered_transcriptome_sorted-read.map.gz'),kind='output')
    ...
    if ( (options.homolog > 0 or (not options.skip_ambigous_filtering)) and
        ...
        job.add('--solexa1.3-quals',kind='parameter')
        job.add('--best',kind='parameter')
        job.add('--suppress','4,5,6,7,8',kind='parameter')
        job.add('--chunkmbs','128',kind='parameter')
        job.add('',datadir('transcripts_index/'),kind='input')
        ...
        job.add('2>',outdir('log_bowtie_reads_filtered_all-possible-mappings-transcriptome_map.stdout.txt'),kind='output',checksum='no',temp_path = temp_flag)
        #job.add('2>&1',kind='parameter',checksum='no')
        #job.run() #XXX
        ...
        job.add('find_homolog_genes.py',kind='parameter')
        ...
        job.add('--output_offending_pair_reads',outdir('list_offending_reads.txt'),kind='output')
        job.run()
        ...
        if job.run():
            ...
            crgt = float(crgt.pop(0).split(tag)[1].strip())
            crgt = int(crgt * options.homolog)
        ...
        if job.iff(not empty(outdir('reads-filtered_multiple-mappings-genome.fq')),id="#reads-filtered_multiple-mappings-genome.fq#"):
            ...
            job.add('2>',outdir('log_bowtie_reads_filtered_all-possible-mappings-transcriptome_multiple_map.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
        ...
        if not options.skip_ambigous_filtering:
...
#            job.run()
            ...
            job.link(outdir('reads_filtered_transcriptome_sorted-read.map'), outdir('reads_filtered_transcriptome_sorted-read_no-offending-reads.map'))
    else:
        job.link(outdir('reads_filtered_transcriptome_sorted-read.map'), outdir('reads_filtered_transcriptome_sorted-read_no-offending-reads.map'))
    ...
    job.add('--label','no_protein_product',kind='parameter')
    job.add('--similar_gene_symbols',kind='parameter')
    job.add('--filter_genes',datadir('genes_with_no_proteins.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('paralogs.txt'),kind='output')
    job.add('--output_fusion_genes',outdir('candidate_fusion-genes_02.txt'),kind='output')
    job.run()
    # label fusion genes -- readthrough
    job.add('label_fusion_genes.py',kind='program')
    ...
    job.add('--filter_gene_pairs',datadir('adjacent_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ensembl_fully_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ensembl_partially_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ensembl_same_strand_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('pseudogenes.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('rrnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('trnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('mirnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('lincrnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('mt.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('snornas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('snrnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('yrnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('7skrnas.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('antisenses.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('pairs_pseudogenes.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('ribosomal_proteins.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('oncogenes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('known.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('cosmic.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('chimerdb2.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('cgp.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('conjoing.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ticdb.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('rp11.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('ctd.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('ctc.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('rp.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('healthy.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('cacg.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ucsc_fully_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ucsc_partially_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('ucsc_same_strand_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('refseq_fully_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('refseq_partially_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('refseq_same_strand_overlapping_genes.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('dgd.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('tcga.txt'),kind='output')
    ...
    job.add('--filter_gene_pairs',datadir('bodymap2.txt'),kind='output')
    ...
    job.add('--filter_genes',datadir('metazoa.txt'),kind='output')
    job.add('--output_fusion_genes',outdir('candidate_fusion-genes_47.txt'),kind='output')
    job.run()
    #
    last_candidate_file = outdir('candidate_fusion-genes_47.txt')
    ...
    if not options.skip_known_fusions:
        job.add('--allowed_labels','known_fusion,cosmic,chimerdb2,ticdb,cgp',kind='parameter') # it allows the known fusions to be considered for further analysis
...
#        job.clean(outdir('reads_filtered_transcriptome_sorted-read.map'),temp_path=temp_flag)
    ...
    if job.iff( not empty(outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome.fq')),
        ...
        if options.mismatches < 3:
            ...
            job.add('2>',outdir('log_bowtie_reads-unmapped-filtered-out-transcriptome.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
            #job.add('2>&1',kind='parameter',checksum='no') # XXX
            #job.run()
            job.add('|',kind='parameter')
            job.add('LC_ALL=C',kind='parameter')
...
#            job.add('','/dev/null',kind='parameter')
            job.add('2>',outdir('log_bowtie_reads-unmapped-filtered-out-genome.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
            #job.add('2>&1',kind='parameter',checksum='no')
            job.run()
    ...
    #        job.add('--str',outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_end-f4-str.fq'),kind='output',temp_path = temp_flag)
        ...
        if not options.skip_viruses_filtering:
            ...
            if options.keep_viruses:
                ...
                #job.add('--best',kind='parameter')
                #job.add('--strata',kind='parameter')
                job.add('--trim3','3',kind='parameter') # trim on the fly 5bp from 3' end
                job.add('--trim5','3',kind='parameter') # trim the 5
                ...
                job.add('2>',outdir('log_sam-viruses.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
                job.add('|',kind='parameter')
...
#            job.add('--tryhard',kind='parameter')
            ...
            job.add('-k','1,1',kind='parameter')
            job.add('-T',tmp_dir,kind='parameter',checksum='no')
            job.add('>',outdir('viruses_statistics.txt'),kind='output')
            job.run()


            info(job,
                 fromfile = outdir('viruses_statistics.txt'),
                 tofile = info_file,
                 top = ["","","",
...
#        job.add('','/dev/null',kind='parameter')
        job.add('2>',outdir('log_bowtie_reads-unmapped-filtered-out-genome_last.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
        #job.add('2>&1',kind='parameter',checksum='no')
        job.run()
...
#        job.run()
    ...
    if candidates:
        ...
        job.add('seqtk',kind='program')
        job.add('subseq',kind='parameter')
        job.add('',outdir('original.fq.gz'),kind='input',temp_path=temp_flag)
...
#            job.run()
            ...
            job.add('',outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_final.txt'),kind='input',temp_path=temp_flag)
        ...
        if nucleotides_ee > options.limit_bowtie:
            ...
            for i,part in enumerate(parts):
                ...
                job.add('2>',outdir('log_bowtie_reads_mapped-exon-exon-fusion-genes_map.stdout.txt.'+str(i)),kind='output',checksum='no',temp_path=temp_flag)
                #job.add('2>&1',kind='parameter',checksum='no')
                #job.run()
                job.add('|',kind='parameter')
            ...
            job.add('',outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_final_plus.fq'),kind='input',temp_path=temp_flag)
            #job.add('',outdir('reads_mapped-exon-exon-fusion-genes.map'),kind='output') # <== mappings on exon-exon junctions for fusion genes ####### # XXX
            job.add('2>',outdir('log_bowtie_reads_mapped-exon-exon-fusion-genes_map.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
            #job.add('2>&1',kind='parameter',checksum='no')
            #job.run()
            job.add('|',kind='parameter')
            # sort the reads' mappings on exon-exon by reference sequence, i.e.
    ...
    if ((not options.skip_blat) or (not options.skip_star) or (not options.skip_bowtie2)) and candidates:
...
#                         "of FusionCatcher and running it again might help!"))
            ...
            job.add('subseq',kind='parameter')
            job.add('',outdir('original_important.fq.gz'),kind='input')
            job.add('',outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome2.txt'),kind='input',temp_path=temp_flag)
...
#        job.run()
        ...
        if job.iff(empty(outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_psl-p.fq')),
            ...
            job.add('printf',kind='program')
            job.add(('"\nReads Counts before BLAT/STAR/BOWTIE2 alignment:\n'+
                     '--------------------------------------\n"'),kind='parameter')
            job.add('>>',info_file,kind='output')
            job.run()
            job.add('cat',kind='program')
...
#                job.run()
                ...
                if nucleotides_ggu > options.limit_bowtie:
                    ...
                    for i,part in enumerate(parts):
                        ...
                        job.add('2>',outdir('log_bowtie_psl_all.txt'),kind='output',checksum='no',temp_path=temp_flag)
                        #job.add('2>&1',kind='parameter',checksum='no')
                        #job.run()
                        job.add('|',kind='parameter')
                        job.add('sort',kind='parameter')
...
#                         "of FusionCatcher and running it again might help!"))
                ...
                job.add('seqtk',kind='program')
                job.add('subseq',kind='parameter')
                job.add('',outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_psl-pp.fq'),kind='input',temp_path=temp_flag)
                job.add('',outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_psl_all_uniq.map'),kind='input',temp_path=temp_flag)
                job.add('|',kind='parameter')
                ...
                job.add('printf',kind='program')
                job.add(('"\nReads Counts after BLAT/STAR/BOWTIE2 prefiltering (and before BLAT/STAR/BOWTIE2 alignment):\n'+
                         '------------------------------------------------------------------------------------------\n"'),kind='parameter')
                job.add('>>',info_file,kind='output')
                job.run()
                job.add('cat',kind='program')
...
#                job.run()
                ...
                if nucleotides_gg > options.limit_blat:
                    ...
                    job.add('-o',outdir('gene-gene_split.fa'),kind='output')
                    job.run()

                    parts = [el.strip() for el in file(outdir('gene-gene_split.fa'),'r').readlines()]
                    for i,part in enumerate(parts):
                        # file size
                        job.add('du',kind='program')
                    ...
                    job.clean(outdir('gene-gene_split.fa'),temp_path=temp_flag)
                    job.clean(outdir('reads_gene-gene.fa'),temp_path=temp_flag if options.skip_star and options.skip_bowtie2 else 'no')
                    ...
                    for tfile in job.job.genegeneblat:
                ...
                job.add('--anchor2',length_anchor2,kind='parameter')
                job.add('--input_candidate_fusion_genes_reads',outdir('candidate_fusion-genes_no-offending-reads_supporting_paired-reads.txt'),kind='input',temp_path=temp_flag if options.skip_star and options.skip_bowtie2 else 'no')
                job.add('--input_fastq',outdir('original_important.fq.gz'),kind='input',temp_path=temp_flag if options.skip_star and options.skip_bowtie2 else 'no')
            ...
            if not options.skip_star:
                ...
                # changing "/1" and "/2" into "-1" "-2" such that STAR does not touch them
                job.add('sed',kind='program')
                job.add("""'s/\/\([1-2]$\)/\-\\1/;n;n;n'""",outdir('reads_gene-gene_no-str.fq'),kind='input',temp_path=temp_flag if options.skip_bowtie2 else 'no')
                job.add('>',outdir('reads_gene-gene_no-str_fixed.fq'),kind='output')
                job.run()


                sdjboverhang = options.trim_psl_3end_keep
                if job.run():
                ...
                if nucleotides_gg > options.limit_star:
                    ...
                    for i,part in enumerate(parts):
                        ...
                        t = "[from file: '%s']" % (part+'.len',)
                        genomesaindexnbases = int(min(14, math.log(100,2)/(float(2) - 1)))
                        if job.run():
                            t = file(part+'.len','r').readline().strip()
                            genomesaindexnbases = int(min(14, math.log(t,2)/(float(2) - 1)))
                        ...
                        gdr = "%s_star-res/" % (part,)
                        job.add('STAR',kind='program')
                        job.add('--sjdbOverhang',sdjboverhang,kind='parameter')
                        ...
                        job.add('--genomeFastaFiles',part,kind='input',temp_path=temp_flag)
                        ...
                        job.add('--outFilterMatchNmin','30',kind='parameter')
                        job.add('--outFilterMatchNminOverLread','0.90',kind='parameter')
                        job.add('--genomeDir',gd,kind='input',temp_path=temp_flag)
                        job.add('--runThreadN',options.processes,kind='parameter',checksum='no')
                        job.add('--seedSearchStartLmax','20',kind='parameter')# default is: 50
                        job.add('--alignSJoverhangMin','9',kind='parameter')# default is 5?
                    ...
                    job.sink(job.genegenestar, outdir('gene-gene-star.psl.txt'))

                    job.add('concatenate.py',kind='program')
                    job.add('-f',outdir('gene-gene-star.psl.txt'),kind='input',temp_path=temp_flag)
                    #job.add_list('',job.genegenestar,kind='input',temp_path=temp_flag,command_line='no')
                    job.add('',outdir('gene-gene-star.psl'),kind='output')
                    job.run()

                    for tfile in job.job.genegenestar:
                        job.clean(tfile,temp_path=temp_flag)
...
#                    job.run()
                ...
                # find the best unique alignments of reads
                job.add('psl_best_unique_contigs.py',kind='program')
                job.add('--input',outdir('gene-gene-star.psl'),kind='input',temp_path=temp_flag)
                ...
                if not options.all_reads_junction:
                    ...
                    job.add('--input_transcriptome',outdir('reads_filtered_transcriptome_sorted-read_end_important.map'),kind='input',temp_path=temp_flag if options.skip_bowtie2 else 'no')
                ...
                job.add('--input_genegene_fasta',outdir('gene-gene.fa'),kind='input',temp_path=temp_flag if options.skip_bowtie2 else 'no')
                job.add('--input_hugo',datadir('genes_symbols.txt'),kind='input')
                job.add('--input_genes_positions',datadir('genes.txt'),kind='input')
                ...
                job.add('--anchor2',length_anchor2,kind='parameter')
                job.add('--input_candidate_fusion_genes_reads',outdir('candidate_fusion-genes_no-offending-reads_supporting_paired-reads.txt'),kind='input',temp_path=temp_flag if options.skip_bowtie2 else 'no')
                job.add('--input_fastq',outdir('original_important.fq.gz'),kind='input',temp_path=temp_flag if options.skip_bowtie2 else 'no')
            ...
            if not options.skip_bowtie2:
                ...
                if nucleotides_gg > options.limit_bowtie2:
                    ...
                    for i,part in enumerate(parts):
                        ...
                        job.add('-L',length_anchor_bowtie2,kind='parameter')
                        ...
                        job.add('2>',outdir('log_bowtie2_reads-gene-gene.stdout.txt.')+str(i),kind='output',checksum='no',temp_path=temp_flag)
                        job.run()
                        ...
                        job.add('analyze_bowtie2.py',kind='program')
                        job.add('--input',outdir('gene-gene-bowtie2.psl.')+str(i),kind='input',temp_path=temp_flag)
                        job.add('--output',outdir('gene-gene-bowtie2_final.psl.')+str(i),kind='output',dest_list='genegenebowtie2')
                        job.run()


                    job.clean(outdir('gene-gene_split.fa'),temp_path=temp_flag)
                    job.clean(outdir('reads_gene-gene_no-str.fq'),temp_path=temp_flag)
                    job.sink(job.genegenebowtie2, outdir('gene-gene-bowtie2_final.psl.txt'))

                    job.add('concatenate.py',kind='program')
                    job.add('-f',outdir('gene-gene-bowtie2_final.psl.txt'),kind='input',temp_path=temp_flag)
                    job.add('',outdir('gene-gene-bowtie2_final.psl'),kind='output')
                    job.run()
                    ...
                    job.add('-L',length_anchor_bowtie2,kind='parameter')
                    #
    #                job.add('-D','20',kind='parameter')
    #                job.add('-R','3',kind='parameter')
    #                job.add('-N','0',kind='parameter')
    #                job.add('-i','S,1,0.50',kind='parameter')
                    ...
                    job.add('-U',outdir('reads_gene-gene_no-str.fq'),kind='input',temp_path=temp_flag)
                    job.add('-S',outdir('gene-gene-bowtie2.sam'),kind='output')
                    job.add('2>',outdir('log_bowtie2_reads-gene-gene.stdout.txt'),kind='output',checksum='no',temp_path=temp_flag)
                    job.run()
                ...
                # find the best unique alignments of reads
                job.add('psl_best_unique_contigs.py',kind='program')
                job.add('--input',outdir('gene-gene-bowtie2_final.psl'),kind='input',temp_path=temp_flag)
    ...
    # CLEANING
    #
    job.clean(outdir('all_ambiguous_genes.txt'),temp_path=temp_flag)
    job.clean(outdir('candidate_fusion-genes_exon-exon.txt'),temp_path=temp_flag)
    job.clean(outdir('candidate_fusion-genes_further.txt'),temp_path=temp_flag)
    job.clean(outdir('candidate_fusion-genes_missing_mates.txt'),temp_path=temp_flag)
    job.clean(outdir('candidate_fusion-genes_no-offending-reads_supporting_paired-reads.txt'),temp_path=temp_flag)
    job.clean(outdir('count_reads_left_after_filtering.txt'),temp_path=temp_flag)
    job.clean(outdir('gene-gene.2bit'),temp_path=temp_flag)
    job.clean(outdir('gene-gene.fa'),temp_path=temp_flag)
    job.clean(outdir('gene-gene-star/'),temp_path=temp_flag)
    job.clean(outdir('gene-gene-bowtie2/'),temp_path=temp_flag)
    job.clean(outdir('gene-gene-star-results/'),temp_path=temp_flag)
    job.clean(outdir('gene-gene_longest.txt'),temp_path=temp_flag)
    job.clean(outdir('gene-gene_unique.fa'),temp_path=temp_flag)
    job.clean(outdir('original.fq'),temp_path=temp_flag)
    job.clean(outdir('original.fq.gz'),temp_path=temp_flag)
    job.clean(outdir('original_important.fq.gz'),temp_path=temp_flag)
    job.clean(outdir('original_important.txt'),temp_path=temp_flag)
    job.clean(outdir('reads_filtered_mapped-transcriptome.fq'),temp_path=temp_flag)
    job.clean(outdir('reads-filtered_multiple-mappings-genome.fq'),temp_path=temp_flag)
    job.clean(outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_psl-p.fq'),temp_path=temp_flag)
    ...
    job.clean(outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_final.fq'),temp_path=temp_flag)
    job.clean(outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome.fq'),temp_path=temp_flag)
    job.clean(outdir('reads_filtered_transcriptome_sorted-read_end.map'),temp_path=temp_flag)
    job.clean(outdir('reads_filtered_transcriptome_sorted-read_end_important.map'),temp_path=temp_flag)
    job.clean(outdir('reads_gene-gene.fq'),temp_path=temp_flag)
    job.clean(outdir('exon-exon_junction_cut__seq.txt'),temp_path=temp_flag)
    job.clean(outdir('exon-exon_junction_cut__nuc.txt'),temp_path=temp_flag)
    job.clean(outdir('gene-gene_unique__nuc.txt'),temp_path=temp_flag)
    job.clean(outdir('gene-gene__nuc.txt'),temp_path=temp_flag)
    job.clean(outdir('gene-gene__seq.txt'),temp_path=temp_flag)
    job.clean(outdir('gene-gene_unique__seq.txt'),temp_path=temp_flag)
    job.clean(outdir('log_lengths_original_reads.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads_filtered_all-possible-mappings-transcriptome_map.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads_filtered_all-possible-mappings-transcriptome_multiple_map.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads-filtered-out.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads_mapped-exon-exon-fusion-genes_map.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads_mapped-genome.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads_not-mapped-genome_but_mapped-transcriptome.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads_unique-mapped-genome_mapped-transcriptome.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads-unmapped-filtered-out-genome-blat_last.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads-unmapped-filtered-out-genome_last.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads-unmapped-filtered-out-genome.stdout.txt'),temp_path=temp_flag)
    job.clean(outdir('log_bowtie_reads-unmapped-filtered-out-transcriptome.stdout.txt'),temp_path=temp_flag)
    ...
    job.clean(outdir('log_number_of_reads_processed.txt'),temp_path=temp_flag)
    job.clean(outdir('log_overlaps_error.txt'),temp_path=temp_flag)
    job.clean(outdir('log_lengths_reads_gene-gene_no-str.txt'),temp_path=temp_flag)
    job.clean(outdir('log_counts_reads_gene-gene_no-str.txt'),temp_path=temp_flag)
    job.clean(outdir('candidate_fusion-genes_further_paired-reads.txt'),temp_path=temp_flag)
    job.clean(outdir('reads_filtered_not-mapped-genome_not-mapped-transcriptome_final2.txt'),temp_path=temp_flag)
    job.clean(outdir('log_removed_single_reads1.txt'),temp_path=temp_flag)


    job.clean(outdir('restart.sh'),temp_path=temp_flag)
